...
function normalizeTtsConfig(input = {}) {
  ...
  return {
    ...
    rate: Math.max(0.5, Math.min(2, numberOr(source.rate, 1))),
    pitch: Math.max(0, Math.min(2, numberOr(source.pitch, 1))),
    volume: Math.max(0, Math.min(1, numberOr(source.volume, 1))),
    cooldownMs: Math.max(0, Math.min(60000, Math.round(numberOr(source.cooldownMs, 1200)))),
    maxQueue: Math.max(1, Math.min(100, Math.round(numberOr(source.maxQueue, 20)))),
    maxCommentLength: Math.max(20, Math.min(1000, Math.round(numberOr(source.maxCommentLength, 220)))),
