...
function parseNvidiaSmi(output) {
  ...
  for (const line of String(output || "").split(/\r?\n/).map((entry) => entry.trim()).filter(Boolean)) {
    ...
    const totalMb = finite(memoryTotalMb);
    rows.push({
      name,
      vendor: "NVIDIA",
      memoryTotalMb: totalMb,
      memoryMb: totalMb,
      memoryGb: totalMb !== null ? totalMb / 1024 : null,
      driverVersion,
...
async function enrichHardware(input = {}) {
  ...
  hardware.gpu = existing[preferred?.index ?? 0] || null;
  hardware.preferredGpu = existing[preferred?.index ?? 0] || null;
  hardware.hasDedicatedGpu = existing.some((gpu) => gpu.dedicated && !gpu.integrated);
  return hardware;
}
