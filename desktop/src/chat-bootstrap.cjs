...
function registerChatIpc() {
  ...
  ipcMain.handle("cng:save-config", async (_event, input = {}) => {
    ...
    if (safe.chat) { safe.chat.obsChatToken = ""; if (safe.chat.url) { const url = new URL(safe.chat.url); url.searchParams.delete("obsChatToken"); safe.chat.url = url.toString(); } }
...
app.whenReady().then(async () => {
  ...
  core.on("messages", (batch) => broadcast("chat:messages", batch));
  core.on("status", (status) => broadcast("chat:status", status));
  core.on("cleared", (platform) => broadcast("chat:cleared", platform));
