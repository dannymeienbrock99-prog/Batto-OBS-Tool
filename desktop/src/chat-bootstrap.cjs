...
app.whenReady().then(async () => {
  ...
  core.registerAdapter(new TwitchAdapter()); core.registerAdapter(new CngUnifiedAdapter()); core.registerAdapter(new TikTokAdapter()); core.registerAdapter(new YouTubeAdapter());
  ...
  await windows.loadSettings(); registerChatIpc(); globalShortcut.register("CommandOrControl+Shift+C", () => windows.toggle());
  if (main) { main.webContents.once("did-finish-load", () => attachEmbeddedChat(main)); if (!main.webContents.isLoading()) setTimeout(() => attachEmbeddedChat(main), 250); }
