...
(() => {
  ...
  function bind(){
    ...
    root.querySelector("#cfg-tiktok-connect").onclick=async()=>{try{await api.chatConnect("tiktok",{username:value("cfg-tiktok-user")});await refreshSettings()}catch(e){setStatus(e.message,true)}};
  ...
  function bindListenersOnce(){
    ...
    api.onChatStatus((status)=>setStatus(status?.error?`${platformMeta[status.platform]?.[0]||status.platform}: ${status.error}`:"Verbindungen aktualisiert",Boolean(status?.error)));
