...
(() => {
  ...
  const ROLE_PRIORITY = ["broadcaster","moderator","vip","subscriber","follower","viewer"];
  const ROLE_LABELS = { broadcaster:"Broadcaster", moderator:"Moderator", vip:"VIP", subscriber:"Subscriber", follower:"Follower", viewer:"Viewer" };
  const DEFAULT_ROLE_COLORS = {
    broadcaster:{name:"#ff626f",message:"#ffe0e3"}, moderator:{name:"#59e391",message:"#d9ffea"}, vip:{name:"#ff63d8",message:"#ffe0f7"},
    subscriber:{name:"#b98cff",message:"#eee4ff"}, follower:{name:"#58cfff",message:"#dff6ff"}, viewer:{name:"#c8d1dc",message:"#dbe3ee"}
  };
  let filter = "all";
  let messages = [];
  ...
  function loadRoleColors(){
    try {
      const stored = JSON.parse(localStorage.getItem("batto.multiChat.roleColors") || "{}");
      return Object.fromEntries(ROLE_PRIORITY.map((role)=>[role,{...DEFAULT_ROLE_COLORS[role],...(stored[role]||{})}]));
    } catch { return structuredClone(DEFAULT_ROLE_COLORS); }
  }
  function saveRoleColors(){localStorage.setItem("batto.multiChat.roleColors",JSON.stringify(roleColors));updateBody()}
  function twitchRole(message){
    if(message.platform!=="twitch") return "viewer";
    const available = new Set([String(message.role||"").toLowerCase(),...(message.badges||[]).map((v)=>String(v).toLowerCase())]);
    return ROLE_PRIORITY.find((role)=>available.has(role)) || "viewer";
  }
  function render(){
    root.innerHTML = `<div class="multi-chat">
      ...
      <section class="chat-compose"><textarea id="chat-input" maxlength="1000" placeholder="Nachricht schreiben … (Versand wird pro Plattform später separat freigeschaltet)"></textarea><div class="compose-row"><small>Anzeige und TTS laufen lokal.</small><button class="send-btn" id="chat-send" disabled>Senden</button></div></section>
  ...
  function bind(){
    ...
    root.querySelectorAll(".role-color-input").forEach((input)=>input.oninput=()=>{const role=input.dataset.role;const kind=input.dataset.kind;roleColors[role][kind]=input.value;saveRoleColors()});
  ...
  async function refreshSettings(){
    ...
    root.querySelector("#chat-status-grid").innerHTML=Object.entries(statuses).map(([p,s])=>`<div class="status-card"><strong>${esc(platformMeta[p]?.[0]||p)}</strong><small><span class="dot ${s.connected?'on':''}"></span>${s.connected?'Verbunden':s.configured?'Konfiguriert':'Getrennt'}</small></div>`).join("");
    ...
    const tts=await api.getTtsConfig(); if(tts){ttsConfig={...ttsConfig,...tts};root.querySelector("#tts-enabled").checked=ttsConfig.enabled;root.querySelector("#tts-language").value=ttsConfig.language;root.querySelector("#tts-rate").value=ttsConfig.rate;root.querySelector("#tts-pitch").value=ttsConfig.pitch;root.querySelector("#tts-volume").value=ttsConfig.volume;root.querySelector("#tts-cooldown").value=ttsConfig.cooldownMs;root.querySelector("#tts-maxlen").value=ttsConfig.maxCommentLength}
    loadVoices();
  }
  ...
  function updateBody(){const body=root.querySelector("#chat-body");if(!body)return;const visible=messages.filter(m=>filter==="all"||m.platform===filter).slice(-300);root.querySelector("#chat-count").textContent=`${visible.length} Nachrichten`;if(!visible.length){body.innerHTML='<div class="chat-empty"><div><strong>Noch keine Nachrichten</strong><br><small>Verbinde Twitch/CNG/TikTok oder YouTube, um alles in einem Verlauf zu sehen.</small></div></div>';return}body.innerHTML=visible.map(m=>{const meta=platformMeta[m.platform]||platformMeta.cng;const role=twitchRole(m);const colors=m.platform==="twitch"?roleColors[role]:null;const badgeText=(m.badges||[]).map(b=>`<span class="chat-role">${esc(b)}</span>`).join("");return `<div class="chat-row"><span class="platform-badge" style="background:${esc(m.color||meta[1])}">${meta[2]}</span><div><div class="chat-meta"><span class="chat-user" style="color:${esc(colors?.name||m.color||meta[1])}">${esc(m.username)}</span><span class="chat-role">${esc(meta[0])}</span>${m.platform==="twitch"?badgeText:(m.role?`<span class="chat-role">${esc(m.role)}</span>`:"")}</div><div class="chat-message" style="color:${esc(colors?.message||'#dbe3ee')}">${esc(m.message)}</div></div></div>`}).join("");body.scrollTop=body.scrollHeight}
