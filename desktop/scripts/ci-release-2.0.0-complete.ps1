...
$Temporary = Join-Path $PSScriptRoot "ci-release-2.0.0.complete.generated.ps1"
...
& $ElectronExe "scripts/generate-brand-icon.cjs"
...
$ChatReplacement = $ChatPatch + [Environment]::NewLine + 'Invoke-Checked -FilePath "node" -ArgumentList @("scripts/patch-2.0.0-hologram.cjs") -FailureMessage "Hologramm-Persistenz-Patch fehlgeschlagen"'
if (-not $Content.Contains($ChatPatch)) { throw "Hologramm-Patchpunkt im Release-Skript wurde nicht gefunden." }
