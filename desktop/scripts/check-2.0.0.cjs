...
const required = [
  ...
  "src/services/chat-core.cjs","src/services/chat-bot.cjs","src/services/chat-filter.cjs","src/services/moderation-store.cjs","src/services/moderation-bootstrap.cjs",
  "src/services/v4-config-store.cjs","src/services/v4-log-store.cjs","src/services/v4-operations.cjs","src/services/v4-bootstrap.cjs","src/services/stream-status.cjs","src/services/internet-test.cjs",
  ...
  "test/v4-phase1.test.cjs","test/v4-completion.test.cjs","test/chat-bot.test.cjs","test/multi-chat.test.cjs","build/installer.nsh","build/license.txt","package.json"
];
...
for (const relative of ["src/renderer/assets/multi-chat-hero.jpg","src/services/hardware.cjs","src/services/recommendation.cjs","src/services/telemetry.cjs","modules/encoder-monitoring-overlay","../.github/workflows/encoder-monitoring-overlay.yml"]) if (exists(relative)) fail(`Entfernter Bereich ist wieder vorhanden: ${relative}`);
...
if (!String(pkg.scripts?.test || "").includes("v4-completion.test.cjs")) fail("V4-Komplettabnahmetest ist nicht eingebunden.");
...
const operations = read("src/services/v4-operations.cjs");
...
const filter = read("src/services/chat-filter.cjs");
...
must(main, "stream-status.cjs", "Kompakte Stream-Statusleiste wird nicht versorgt.");
...
for (const bridge of ["getV4Configs","testV4Module","getV4Media","importV4Media","getCohostStatus","exportV4Backup","importV4Backup","testChatFilter","getModerationState"]) must(preload, bridge, `V4 Bridge fehlt: ${bridge}`);
...
must(config, "new Set(MODULES.map", "Nicht alle V4-Module sind als lokal implementiert markiert.");
for (const label of ["HILFE","TESTEN","ZURÜCKSETZEN","ÜBERNEHMEN","SPEICHERN","DATEIEN IMPORTIEREN","POOL IM CHAT BOT SPEICHERN","BACKUP EXPORTIEREN","HTTP KOPIEREN","FILTER TESTEN"]) must(settings, label, `V4-Bedienelement fehlt: ${label}`);
mustNot(settings, /textarea[^>]*json|JSON-Pflicht/i, "V4-Einstellungen verlangen wieder eine JSON-Eingabe.");
must(operations, "buildCohostLayout", "Co-Host-Layout-Kern fehlt.");
must(operations, "127.0.0.1", "Co-Host-HTTP-Dienst ist nicht auf Loopback begrenzt.");
must(operations, "writeBackup", "V4-Backup-Export fehlt.");
must(operations, "restoreBackup", "V4-Backup-Import fehlt.");
must(operations, "importMedia", "V4-Medienimport fehlt.");
must(operations, "liveToolsStatus", "LIVE-Tools-Verfügbarkeitsprüfung fehlt.");
must(bootstrap, /new ChatCore\(/, "Multi-Chat Core fehlt.");
must(bootstrap, /new ChatBotService\(/, "Chat Bot fehlt.");
must(bootstrap, /new ChatFilterService\(/, "Chat-Filter fehlt.");
must(bootstrap, "platformEvent", "Plattform-Events werden nicht in den Chat Bot überführt.");
must(bootstrap, "document.getElementById('multi-chat-root')", "Doppeltes Multi-Chat-Dock wird nicht verhindert.");
must(filter, "filterBatch", "Chat-Filter wird nicht als Nachrichtenpipeline bereitgestellt.");
...
console.log(`Batto OBS Tool 2.0.0 – V4 Komplett-Quellvertrag bestanden (${required.length} Kern-Dateien).`);
