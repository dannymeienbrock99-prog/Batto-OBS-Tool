...
const required = [
  "src/main-v2.cjs", "src/main.cjs", "src/preload.cjs", "src/chat-bootstrap.cjs", "src/deck-bootstrap.cjs", "src/deck-creatorhub-bootstrap.cjs", "src/stream-overlay-bootstrap.cjs",
  "src/services/common.cjs", "src/services/deck-store.cjs", "src/services/plugin-registry.cjs", "src/services/store.cjs", "src/services/secret-store.cjs", "src/services/obs-websocket.cjs", "src/services/connection-manager.cjs", "src/services/hybrid-runtime.cjs", "src/services/tiktok-live-studio.cjs", "src/services/stream-overlay-server.cjs",
  "src/services/platforms/tiktok-adapter.cjs", "src/services/platforms/twitch-adapter.cjs", "src/services/platforms/youtube-adapter.cjs", "src/services/platforms/cng-adapter.cjs",
  "src/renderer/index.html", "src/renderer/app.js", "src/renderer/commercial-settings.js", "src/renderer/commercial-settings.css", "src/renderer/multi-chat.js", "src/renderer/multi-chat.css", "src/renderer/touch-deck-20260802.js", "src/renderer/touch-deck-20260802.css", "src/renderer/integration-20260904.js", "src/renderer/integration-20260904.css",
...
for (const marker of ["touch-deck-20260802.css","touch-deck-20260802.js","integration-20260904.css","integration-20260904.js","commercial-settings.css","commercial-settings.js","multi-chat.css","multi-chat.js","view-overview","view-obs","multi-chat-root","view-deck-0802","view-moderation","view-cohost","view-holo","view-settings"]) {
...
for (const marker of ["SAFE_INVOKE_CHANNELS", '"deck:execute-button"', '"deck:open-original-0802"', '"deck:original-0802-status"', "onStateChanged", 'ipcRenderer.invoke("chat:unified-clear"']) {
...
for (const marker of ["stream-overlay-bootstrap.cjs","deck-bootstrap.cjs","deck-creatorhub-bootstrap.cjs","chat-bootstrap.cjs"]) if (!entry.includes(marker)) throw new Error(`2.1-Einstieg lädt Runtime-Modul nicht: ${marker}`);
...
for (const marker of ["startStreamOverlay","ensureChatObs","twitch-oauth-token","youtube-oauth-token"]) if (!chat.includes(marker)) throw new Error(`Multi-Chat-Produktverdrahtung fehlt: ${marker}`);
const multiChat = fs.readFileSync(path.join(root, "src", "renderer", "multi-chat.js"), "utf8");
for (const marker of ["ws://127.0.0.1:21213/","connectTikfinity","handleTikfinity","batto-moderation-v1","api.onChatMessages"]) if (!multiChat.includes(marker)) throw new Error(`Multi-Chat/TikFinity-Verdrahtung fehlt: ${marker}`);
...
if (!prepareDeck.includes("51be33d29c07f50323b19d58782804af391b8394") || !prepareDeck.includes("dotnet")) throw new Error("Original-TouchDeck wird nicht aus dem festgeschriebenen 02.08.2026-Commit gebaut.");
...
if (!prepareScript.includes("prepare:touchdeck-0802")) throw new Error("Original-TouchDeck wird nicht im Produktionsbuild vorbereitet.");
const resources = JSON.stringify(packageJson.build?.extraResources || []);
if (!resources.includes("touchdeck-0802")) throw new Error("Original-TouchDeck wird nicht in den Installer übernommen.");
