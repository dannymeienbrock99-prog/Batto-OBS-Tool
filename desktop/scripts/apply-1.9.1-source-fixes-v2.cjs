...
app.on("second-instance", () => {
  ...
  mainWindow.focus();
    ...
    const oldModules = `async function startLocalModules() {
  ...
  await holoServer.start();
    ...
    const newModules = `async function startLocalModules() {
  moduleErrors = {};
  const monitoringRoot = path.join(__dirname, "..", "modules", "encoder-monitoring-overlay", "web");
  try {
    monitoringServer = new MonitoringOverlayServer({
...
async function patchRenderer() {
  ...
  await patch("src/renderer/app.js", (source) => {
    ...
    const oldMonitoring = `async function loadMonitoringFrame() {
    try {
    ...
    const newMonitoring = `function applyModuleFrame(frame, status, label) {
  ...
  async function loadMonitoringFrame() {
    ...
    const oldHolo = `async function loadHoloFrame() {
    try {
    ...
    const newHolo = `async function loadHoloFrame() {
    try {
      const status = await api.getHoloStatus();
      byId("holo-url").textContent = status.overlayUrl || "Nicht gestartet";
...
async function verify() {
  ...
  const files = [
    "src/main.cjs", "src/preload.cjs", "src/services/hardware.cjs", "src/services/obs-websocket.cjs",
    "src/services/store.cjs", "src/services/telemetry.cjs", "src/services/twitch-holo-server.cjs",
    "src/renderer/index.html", "src/renderer/app.js", "src/renderer/styles.css",
    "modules/encoder-monitoring-overlay/web/editor.html", "modules/encoder-monitoring-overlay/web/editor.js",
  ...
  if (/show-test-values|Testwerte anzeigen|pathname === "\/api\/test"|createTestTelemetry/.test(monitoring)) throw new Error("Interner Monitoring-Democode ist noch vorhanden.");
  if (/frame-ancestors/.test(await read("modules/encoder-monitoring-overlay/src/server.cjs"))) throw new Error("Monitoring-Editor würde weiterhin blockiert.");
