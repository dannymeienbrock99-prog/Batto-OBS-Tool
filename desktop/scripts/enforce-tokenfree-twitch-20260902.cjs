...
// Twitch path anonymous/read-only too so there is no second OAuth code path in production.
{
  ...
  // No token may be persisted even if an old settings file still contains one.
  if (!text.includes("this.persistSettings?.();")) throw new Error("Tokenfreier Twitch-Connect konnte nicht eingebaut werden.");
  write(file, text);
}
